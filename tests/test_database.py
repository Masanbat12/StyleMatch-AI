...
def test_local_feedback_points_update_existing_outfit(tmp_path, monkeypatch):
    data_dir = tmp_path / "data"
    db_path = data_dir / "saved_outfits.db"
    ...
    database.init_db()
    like_backend = database.record_outfit_feedback(
        user_id="demo-user",
        username="messi",
        skin_tone="medium",
        undertone="neutral",
    ...
    unlike_backend = database.record_outfit_feedback(
        user_id="demo-user",
        username="messi",
        skin_tone="medium",
        undertone="neutral",
    ...
    feedback = database.get_style_feedback("demo-user")

    assert like_backend == "local"
    assert unlike_backend == "local"
    assert feedback["feedback_count"] == 1
    assert feedback["like_count"] == 0
    assert feedback["unlike_count"] == 1
    assert feedback["color_scores"]["navy"] == database.UNLIKE_POINTS
    assert feedback["slot_color_scores"]["shirt"]["navy"] == database.UNLIKE_POINTS
